...
def configure_log_file(playback_type="", log_file=config_dash.LOG_FILENAME):
    ...
    if config_dash.LOG.handlers:
        config_dash.LOG.handlers.clear()
        
    log_formatter = logging.Formatter('%(asctime)s - %(processName)s - %(filename)s:%(lineno)d - %(levelname)s - %(message)s')
    ...
    # Add the handler for the file if present
    if log_file:

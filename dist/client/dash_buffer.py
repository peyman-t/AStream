...
class DashPlayer:
    ...
    def log_entry(self, action, bitrate=0):
        ...
        if self.buffer_log_file:
            ...
            # Encoding strings to bytes for binary write
            if not os.path.exists(self.buffer_log_file):
                header_row = "EpochTime,CurrentPlaybackTime,CurrentBufferSize,CurrentPlaybackState,Action,Bitrate\n".encode('utf-8')
                
            stats_line = ','.join(str_stats) + '\n'
            stats_bytes = stats_line.encode('utf-8')
            
            with open(self.buffer_log_file, "ab") as log_file_handle:
                if header_row:
                    log_file_handle.write(header_row)
                log_file_handle.write(stats_bytes)
